...
new #[Title('Groupe d\'examens'), Layout('layouts::app.other.laboratoire')] class extends Component {
    ...
    #[Computed]
    public function sortedActes(): Collection
    {
        return $this->groupe->actes->sortBy('name')->values();
    }
    ...
    public function save(): void
    {
        ...
        unset($this->sortedActes, $this->montantTotal);
...
<div class="mx-auto max-w-7xl space-y-6 pb-28">
    <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div class="space-y-2">
            ...
            @if (filled($groupe->description))
                <p class="max-w-3xl text-sm text-slate-500 dark:text-slate-400">{{ $groupe->description }}</p>
            @endif
        </div>
    ...
    @if (! $editing)
        <div class="grid gap-4 sm:grid-cols-3">
            <div
                ...
                <p class="text-xs font-black uppercase tracking-[0.22em] text-sky-700 dark:text-sky-300">Examens</p>
                <p class="mt-3 text-3xl font-black text-sky-900 dark:text-sky-100">{{ $groupe->actes_count }}</p>
                <p class="mt-1 text-xs text-sky-700/80 dark:text-sky-300/80">Actes associés au groupe</p>
            </div>
            <div
                ...
                <p class="text-xs font-black uppercase tracking-[0.22em] text-emerald-700 dark:text-emerald-300">Montant</p>
                <p class="mt-3 text-3xl font-black text-emerald-900 dark:text-emerald-100">
                    {{ number_format($this->montantTotal, 2, ',', ' ') }} $
                </p>
                <p class="mt-1 text-xs text-emerald-700/80 dark:text-emerald-300/80">Total cumulé des tarifs</p>
            </div>
            <div
                ...
                <p class="mt-3 text-xl font-black text-slate-900 dark:text-white">
                    {{ $groupe->service?->name ?: 'Tous services' }}
                </p>
                <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">
                    Créé le {{ $groupe->created_at?->format('d/m/Y H:i') ?: '—' }}
                </p>
            </div>
        </div>
    @endif
        ...
        <form wire:submit.prevent="save" class="grid gap-6 xl:grid-cols-[minmax(0,1.4fr)_360px]">
            <div class="space-y-6">
                ...
                <section
                    ...
                    <div class="max-h-112 space-y-4 overflow-y-auto p-5">
                                ...
                                <div class="grid gap-2 md:grid-cols-2">
                                    @foreach ($actes as $acte)
                                        <label wire:key="edit-acte-{{ $acte->id }}"
                                            class="flex cursor-pointer items-start gap-2 rounded-xl border border-slate-200 px-3 py-2 transition hover:border-sky-300 hover:bg-sky-50/50 dark:border-slate-700 dark:hover:border-sky-600/40">
                                            <input type="checkbox" value="{{ $acte->id }}"
                                                wire:model="selectedActes" class="mt-0.5 size-4 rounded text-sky-600" />
                                            <span class="text-sm text-slate-800 dark:text-slate-200">{{ $acte->name }}</span>
                                        </label>
                                    @endforeach
                                </div>
                            </div>
            ...
            <aside class="space-y-3 xl:sticky xl:top-6 xl:self-start">
                <div
                    ...
                    <p class="text-xs font-black uppercase tracking-[0.2em] text-slate-400">Sélection</p>
                    <p class="mt-2 text-2xl font-black text-slate-900 dark:text-white">{{ count($selectedActes) }}</p>
                    <p class="text-xs text-slate-500">examen(s) coché(s)</p>
                </div>
    ...
    @else
        <section
            class="overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-950/70">
            <div
                class="flex flex-col gap-2 border-b border-slate-100 px-5 py-4 dark:border-slate-800 md:flex-row md:items-center md:justify-between">
                <div>
                    <p class="text-xs font-black uppercase tracking-[0.22em] text-slate-400">Composition</p>
                    <h2 class="mt-1 text-xl font-black text-slate-900 dark:text-white">Examens du groupe</h2>
                </div>
                <p class="text-sm text-slate-500 dark:text-slate-400">{{ $groupe->actes_count }} examen(s)</p>
            </div>

            @if ($this->sortedActes->isEmpty())
                <div class="px-5 py-16 text-center">
                    <p class="text-sm font-medium text-slate-600 dark:text-slate-300">Aucun examen associé.</p>
                    <flux:button wire:click="startEditing" variant="primary" color="sky" icon="plus" class="mt-4">
                        Ajouter des examens
                    </flux:button>
                </div>
            @else
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-100 dark:divide-slate-800">
                        <thead class="bg-slate-50/80 dark:bg-slate-900/50">
                            <tr>
                                <th
                                    class="px-5 py-3 text-left text-xs font-black uppercase tracking-[0.16em] text-slate-500">
                                    #</th>
                                <th
                                    class="px-5 py-3 text-left text-xs font-black uppercase tracking-[0.16em] text-slate-500">
                                    Examen</th>
                                <th
                                    ...
                                    Service</th>
                                <th
                                    class="px-5 py-3 text-right text-xs font-black uppercase tracking-[0.16em] text-slate-500">
                                    Tarif</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                            @foreach ($this->sortedActes as $index => $acte)
                                <tr wire:key="show-acte-{{ $acte->id }}"
                                    class="transition hover:bg-slate-50/60 dark:hover:bg-slate-900/40">
                                    <td class="px-5 py-4 text-xs font-semibold text-slate-400">{{ $index + 1 }}</td>
                                    <td class="px-5 py-4">
                                    ...
                                    <td class="px-5 py-4 text-sm text-slate-600 dark:text-slate-300">
                                        {{ $acte->service?->name ?: 'Sans service' }}
                                    </td>
                                    <td
                                        class="px-5 py-4 text-right text-sm font-bold text-sky-700 dark:text-sky-300">
                        ...
                        <tfoot class="bg-slate-50/80 dark:bg-slate-900/50">
                            <tr>
                                ...
                                <td class="px-5 py-4 text-right text-base font-black text-slate-900 dark:text-white">
                                    {{ number_format($this->montantTotal, 2, ',', ' ') }} $
                                </td>
                            </tr>
                        </tfoot>
