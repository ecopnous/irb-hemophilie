...
use App\Services\ReceptionSupplyStockService;
use Flux\Flux;
...
new #[Title('Papeterie reception'), Layout('layouts::app.other.reception')] class extends Component {
    ...
    public string $formCategory = 'papeterie';
    ...
    {
        abort_unless(current_hopital_id(), 403, 'Selectionnez un hopital pour gerer la papeterie.');
    }

    protected function baseQuery()
    {
        return ReceptionSupply::query()->whereHopitalId(current_hopital_id());
    }
    ...
    #[Computed]
    public function viewedSupply(): ?ReceptionSupply
    {
        if (! $this->viewingId) {
            return null;
        }
    ...
    public function saveSupply(): void
    {
        ...
        $validated = $this->validate([
            'formDesignation' => [
                ...
                Rule::unique('reception_supplies', 'designation')
        ...
        if ($this->editingId) {
            ...
            ReceptionSupply::query()->create($payload + ['hopital_id' => $hopitalId]);
    ...
    public function saveMovement(): void
    {
        ...
        try {
            app(ReceptionSupplyStockService::class)->applyMovement(
                $supply,
...
<section class="w-full space-y-6 p-4 md:p-6">
    ...
    <x-header_default
        ...
        :navigations="[
            ...
            ['label' => 'Papeterie', 'icon' => 'clipboard-document-list'],
        ]"
    >
        <x-slot:actions>
            <x-button wire:click="importCatalog">Importer catalogue</x-button>
            <x-button icon="plus" position="left" wire:click="openCreate">Nouvel article</x-button>
        </x-slot>
    </x-header_default>
    ...
    <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <div class="rounded-3xl border border-sky-200 bg-sky-50/80 p-5 shadow-sm dark:border-sky-500/20 dark:bg-sky-500/10">
            <p class="text-xs font-bold uppercase tracking-[0.22em] text-sky-700 dark:text-sky-300">Articles</p>
    ...
    <div class="rounded-3xl border border-slate-200 bg-white/95 p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950/70 md:p-6">
        <livewire:reception-supply-table />
    </div>
