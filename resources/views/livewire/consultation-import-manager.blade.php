<div>
    ...
    <flux:modal wire:model="showModal" class="max-w-2xl">
        <div class="space-y-6">
            <div>
                <flux:heading size="lg">Importer des consultations</flux:heading>
                <flux:subheading>
                    Importez des consultations depuis Excel (.xlsx, .xls) ou CSV. Les patients doivent deja exister dans le systeme.
                </flux:subheading>
            </div>
            ...
            <div class="rounded-2xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-900 dark:border-blue-500/30 dark:bg-blue-500/10 dark:text-blue-100">
                ...
                <ul class="mt-2 list-disc space-y-1 pl-5">
                    <li>Identifiez le patient par <strong>NIN</strong> ou <strong>INS</strong>.</li>
                    <li>Indiquez au moins un <strong>acte</strong> par ligne (separes par virgule).</li>
                    <li>Traitement par lots de 1 000 lignes via file d'attente.</li>
                    <li>Lancez <code class="rounded bg-black/5 px-1 dark:bg-white/10">php artisan queue:work --queue=imports,default</code> en local.</li>
                </ul>
            </div>
                    ...
                    @if ($this->activeImport->isFinished())
                        @if ($this->activeImport->status === 'completed')
                            <flux:callout variant="success" class="mt-4" icon="check-circle">
                                Import termine. {{ number_format($this->activeImport->success_count) }} consultation(s) ajoutee(s).
                            </flux:callout>
                        @else
                        ...
                        @if ($this->activeImport->errors_file_path && $this->activeImport->failed_count > 0)
                            <div class="mt-4">
                                <flux:button
                                    ...
                                    :href="route('consultation.import.errors', $this->activeImport)"
                                >
            ...
            <form wire:submit="startImport" class="space-y-4">
                ...
                <div class="flex flex-wrap items-center justify-between gap-3">
                    <flux:button
                        ...
                        :href="route('consultation.import.template')"
                    >
